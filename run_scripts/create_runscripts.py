...
if True:
    for relevance_graph_class_name in relevance_graph_class_names:
        ...
        run_scripts_dir = f"run_scripts/{relevance_graph_class_name}"
        for f in os.listdir(run_scripts_dir):
            os.remove(os.path.join(run_scripts_dir, f))

        for rel_distance in rel_distances:
            file_name = f"{run_scripts_dir}/starexec_run_PyRes_rd_{rel_distance:03d}"
            runscript_content = f"""#!/bin/tcsh
            #
